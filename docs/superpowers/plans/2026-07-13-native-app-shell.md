...
6. **Android**: download `google-services.json` from the Firebase console and add it to `apps/web/android/app/` — `build.gradle` only applies the `com.google.gms.google-services` plugin when this file is present, so Android push silently doesn't build in the Firebase SDK without it.

## Known gap — native push registration is not wired end-to-end (found in final review, not a manual step, needs a follow-up code task)

Tasks 2/3/5/7 built the `native_push_tokens` table, the backend FCM sender (`apps/api/core/native_push.py`), and installed `@capacitor/push-notifications` on both platforms — but **no task wrote the client-side code that actually requests permission, registers for push, and POSTs the resulting token to Supabase.** `native_push_tokens` will stay empty until this is built, so `send_native_push()` is currently a safe no-op (`price_alerts.py`'s `if user_native_tokens:` guard covers the empty case), not a crash — but native push is inert end-to-end today, independent of the manual steps above.

Separately, verified against `@capacitor/push-notifications@8.1.1`'s source: **iOS registers via a raw APNs device token** (`UIApplication.shared.registerForRemoteNotifications()`, no Firebase iOS SDK involved), while **Android genuinely goes through FCM**. `native_push.py`'s `send_native_push()` calls `firebase_admin.messaging.send(Message(token=token))`, which requires real FCM tokens — a raw APNs token saved from an iOS client would not work there as-is. Fixing this needs either adding Firebase's iOS Messaging SDK client-side (so iOS also produces a real FCM token) or a separate backend path that sends raw APNs directly for iOS tokens. Scope this as its own follow-up task before relying on iOS native push in production.
