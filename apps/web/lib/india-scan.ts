// Shared India momentum scan — extracted from api/ml/signals/route.ts so the
// scan-log-daily cron can run the exact same scan/scoring the Signals page uses,
// instead of duplicating the 100-stock universe and RSI/EMA math a second time.
...
// Zone classification — must stay in sync with scoreSig/ZONE_FROM_CAT in
// dashboard/signals/page.tsx (client computes the same thing for immediate render).
export const ZONE_FROM_CAT: Record<string, string> = {
  buy: 'Strong Momentum', accumulate: 'Building', hold: 'Sideways', sell: 'Weak / Declining',
};

export function scoreSig(s: Signal): 'buy' | 'accumulate' | 'hold' | 'sell' {
