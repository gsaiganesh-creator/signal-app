...
export function SupportContent({ riskDisclosureHref = '/risk-disclosure' }: { riskDisclosureHref?: string }) {
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--surf)', border: '1px solid var(--bdr)', borderRadius: 16, padding: '28px 28px 8px', marginBottom: 28 }}>
        ...
        {[
          ...
          { n: '6', title: 'Explore Forex, Commodities & Sectors', body: 'All available in the More drawer (bottom nav on mobile, Markets tab on desktop). Forex tracks 9 INR pairs. Commodities shows MCX proxy prices for Gold, Silver, Crude, and more.' },
        ].map(step => (
      ...
      <div style={{ background: 'var(--surf)', border: '1px solid var(--bdr)', borderRadius: 16, padding: '28px 28px 8px', marginBottom: 28 }}>
        ...
        <Faq q="How do I upgrade my plan?"
          a={<>Go to Dashboard → Upgrade Plan (Account tab, or the More drawer on mobile). Choose your plan and complete payment via Razorpay. Your plan upgrades instantly after payment verification. Supported methods: UPI, Net Banking, Credit/Debit cards, Wallets.</>} />
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(23,64,245,0.12),rgba(139,92,246,0.08))', border: '1px solid rgba(79,111,250,0.25)', borderRadius: 16, padding: '28px 28px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 20, fontSize: 12, color: 'var(--dim)' }}>
          ...
          <Link href={riskDisclosureHref} style={{ color: 'var(--dim)' }}>Risk Disclosure</Link>
        </div>
      </div>
