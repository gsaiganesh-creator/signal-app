...
export default function UpgradePage() {
  ...
  async function checkPromo() {
    ...
    try {
      ...
      const d = await res.json() as { valid: boolean; discount_pct?: number; label?: string | null; error?: string };
      if (d.valid && d.discount_pct) {
        setPromoApplied({ code, discount_pct: d.discount_pct, label: d.label ?? null });
      } else {
  ...
  async function handleUpgrade(plan: typeof PLANS[number]) {
    ...
    try {
      ...
      const order = await orderRes.json() as {
        ...
        welcome_disc_pct?: number; promo_code?: string | null; promo_disc_pct?: number; original_amount?: number; error?: string;
      };
        ...
        handler: async (resp) => {
          ...
          const verRes = await fetch('/api/payment/verify', {
            ...
            body: JSON.stringify({
              ...
              discount_pct: order.promo_disc_pct || order.welcome_disc_pct || 0,
            }),
          });
          const verify = await verRes.json() as { verified: boolean; plan?: string; expires?: string };
          if (verify.verified) {
            setPromoApplied(null); setPromoInput('');
  ...
  return (
    <>
      ...
      {/* Promo code — one-time codes generated by the founders (Twitter giveaways etc) */}
      <div style={{ background:'var(--card-bg)', border:'1px solid var(--card-bdr)', borderRadius:14, padding:'16px 20px', marginBottom:22 }}>
        {promoApplied ? (
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <div style={{ fontSize:22 }}>🎟️</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, fontWeight:800 }}>
                Code <span style={{ fontFamily:'monospace' }}>{promoApplied.code}</span> applied — <span style={{ color:'var(--grn)' }}>{promoApplied.discount_pct}% off</span>
              </div>
              {promoApplied.label && <div style={{ fontSize:11, color:'var(--dim)', marginTop:2 }}>{promoApplied.label}</div>}
            </div>
            <button onClick={() => { setPromoApplied(null); setPromoInput(''); }}
              style={{ fontSize:11, fontWeight:700, padding:'6px 12px', borderRadius:8, background:'var(--surf2)', border:'1px solid var(--bdr)', color:'var(--dim)', cursor:'pointer', fontFamily:'inherit' }}>
              Remove
            </button>
          </div>
        ) : (
          <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
            <span style={{ fontSize:13, fontWeight:700, whiteSpace:'nowrap' }}>🎟️ Have a promo code?</span>
            <input value={promoInput} onChange={e => setPromoInput(e.target.value.toUpperCase())}
              onKeyDown={e => e.key === 'Enter' && checkPromo()}
              placeholder="Enter code"
              style={{ height:36, padding:'0 12px', borderRadius:9, border:'1px solid var(--card-bdr)', background:'var(--surf2)', color:'var(--txt)', fontSize:13, fontFamily:'monospace', outline:'none', width:160 }}/>
            <button onClick={checkPromo} disabled={promoChecking || !promoInput.trim()}
              style={{ height:36, padding:'0 16px', borderRadius:9, background:'var(--blu)', border:'none', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit', opacity: promoChecking || !promoInput.trim() ? 0.5 : 1 }}>
              {promoChecking ? '…' : 'Apply'}
            </button>
            {promoError && <span style={{ fontSize:12, color:'var(--red)' }}>{promoError}</span>}
          </div>
        )}
      </div>
