...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ background:'var(--bg)', color:'var(--txt)', fontFamily:'Inter,system-ui,sans-serif', minHeight:'100vh' }}>
      ...
      {/* Top Nav */}
      <nav style={{ display:'flex', alignItems:'center', height:58, padding:'0 clamp(12px,3vw,32px)', background:'var(--surf)', borderBottom:'1px solid var(--bdr)', position:'sticky', top:0, zIndex:100 }}>
        ...
        {/* Desktop nav tabs — hidden on mobile via .dash-topnav-links */}
        <DashboardTopNav />
        ...
        <div className="dash-right">
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, padding:'4px 12px 4px 4px', borderRadius:20, background:'var(--surf2)', border:'1px solid var(--bdr)', cursor:'pointer', flexShrink:0 }}>
            ...
            <span className="dash-username" style={{ fontSize:12, fontWeight:600 }}>Vaasudev A.</span>
          </div>
        </div>
      </nav>
      ...
      {/* Mobile bottom tab bar — visible only on ≤900px via CSS */}
      <MobileBottomNav />
    </div>
  );
}
