    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', padding:'15px 18px', gap:14, flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                    <button onClick={() => setExpanded(isExp ? null : g.id)}
                      style={{ height:30, padding:'0 10px', borderRadius:8, background:'var(--surf2)', border:'1px solid var(--bdr)', color:'var(--dim)', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                      {isExp ? '▲' : '▼ Details'}
                    </button>
                    <button onClick={() => openEdit(g)}
                      style={{ height:30, width:30, borderRadius:8, background:'var(--surf2)', border:'1px solid var(--bdr)', color:'var(--dim)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>
                      ✏️
                    </button>
                ...
                {isExp && (
                  <div style={{ borderTop:'1px solid var(--bdr)', background:'var(--surf2)', padding:'16px 18px' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:14, marginBottom: ld?.signals?.length ? 14 : 0 }}>
                      <div style={{ background:'var(--surf)', border:'1px solid var(--bdr)', borderRadius:11, padding:'12px 14px' }}>
                    ...
                    {ld?.signals && ld.signals.length > 0 && (
                      <div>
                        ...
                        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                          {ld.signals.map((s, i) => (
                            <span key={i} style={{ fontSize:11, fontWeight:600, padding:'3px 10px', borderRadius:20, background:'rgba(255,255,255,0.06)', border:'1px solid var(--bdr)' }}>{s}</span>
                          ))}
                        </div>
                      </div>
                    )}
      ...
      {addOpen && (
        <>
          ...
          <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:'92%', maxWidth:540, maxHeight:'90vh', overflowY:'auto', zIndex:310, background:'var(--surf)', border:'1px solid var(--bdr)', borderRadius:20, padding:28 }}>

            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:22 }}>
              <div style={{ fontSize:18, fontWeight:800 }}>{editId ? 'Edit' : 'Add'} Equity Grant</div>
              <button onClick={() => setAddOpen(false)}
                style={{ width:30, height:30, borderRadius:8, background:'var(--surf2)', border:'1px solid var(--bdr)', color:'var(--dim)', cursor:'pointer', fontFamily:'inherit', fontSize:14 }}>✕</button>
            </div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={() => setAddOpen(false)}
                style={{ flex:1, height:44, borderRadius:12, background:'transparent', border:'1px solid var(--bdr)', color:'var(--dim)', fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                Cancel
              </button>
