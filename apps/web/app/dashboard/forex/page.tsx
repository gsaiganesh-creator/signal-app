    ...
    <div style={{ maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={fetchRates} disabled={ratesLoading}
            style={{ height:36, padding:'0 14px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', color:'var(--txt)', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', opacity: ratesLoading ? 0.6 : 1 }}>
      ...
      <div style={{ ...card, marginBottom:20 }}>
        ...
        <div className="g3" style={{ display:'grid', gap:8 }}>
          {LIVE_PAIRS.map(pair => {
            ...
            return (
              <div key={pair.code} style={{ background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:'12px 14px' }}>
      ...
      {showAdd && (
        <div onClick={() => setShowAdd(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:600, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:24, width:'min(420px,95vw)', boxShadow:'0 24px 64px rgba(0,0,0,0.45)' }}>
