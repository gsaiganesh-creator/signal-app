    ...
    <div style={{ maxWidth:1200 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <button onClick={() => fileRef.current?.click()}
            style={{ height:36, padding:'0 16px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(79,111,250,0.22)', color:'var(--txt)', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:6 }}>
            📂 Import File
          </button>
            ...
            <div key={p.id} style={{ position:'relative', display:'inline-flex', alignItems:'center', gap:0 }}>
              {isRenaming ? (
                <>
                  ...
                  <button onClick={() => setRenamingPortId(null)} style={{ height:34, padding:'0 8px', marginLeft:4, borderRadius:8, background:'transparent', border:'1px solid rgba(79,111,250,0.22)', color:'var(--dim)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>✕</button>
                </>
              ) : (
                <>
              ...
              {menuPortId === p.id && menuPortPos && (
                <div style={{ position:'fixed', top:menuPortPos.top, left:menuPortPos.left, zIndex:9999, background:'linear-gradient(145deg,rgba(17,36,80,0.72),rgba(8,14,42,0.82))', border:'1px solid rgba(79,111,250,0.22)', borderRadius:10, boxShadow:'0 8px 32px rgba(0,0,0,0.45)', minWidth:160, padding:'4px 0' }}>
          ...
          {usPortfolios.length === 0 ? (
            ...
            <>
              ...
              <div style={{ display:'flex', gap:8, justifyContent:'center', flexWrap:'wrap' }}>
                ...
                <button onClick={() => fileRef.current?.click()} style={{ height:38, padding:'0 16px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(79,111,250,0.22)', color:'var(--txt)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>📂 Import CSV / XLSX</button>
              </div>
          ...
          <div key={sector} style={{ marginBottom:20 }}>
            ...
            <div className="g4" style={{ display:'grid', gap:8 }}>
              {picks.map(pick => {
                ...
                return (
                  <div key={pick.sym} style={{ background:'var(--surf2)', border:'1px solid rgba(79,111,250,0.22)', borderRadius:10, padding:'10px 12px', borderLeft:`3px solid ${isPos ? 'var(--grn)' : 'var(--red)'}` }}>
      ...
      {addOpen && (
        <div onClick={() => setAddOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:600, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:'linear-gradient(145deg,rgba(17,36,80,0.72),rgba(8,14,42,0.82))', border:'1px solid rgba(79,111,250,0.22)', borderRadius:18, padding:24, width:'min(400px,95vw)', boxShadow:'0 24px 64px rgba(0,0,0,0.45)' }}>
      ...
      {selectedHolding && (() => {
        ...
        function Stat({ label, val, sub, color }: { label:string; val:string; sub?:string; color?:string }) {
          return (
            <div style={{ background:'var(--surf2)', border:'1px solid rgba(79,111,250,0.22)', borderRadius:9, padding:'9px 12px' }}>
        ...
        return (
          <div onClick={() => { setSelected(null); setDetail(null); }}
            ...
            <div onClick={e => e.stopPropagation()}
              style={{ background:'linear-gradient(145deg,rgba(17,36,80,0.72),rgba(8,14,42,0.82))', border:'1px solid rgba(79,111,250,0.22)', borderRadius:20, padding:24, width:'min(560px,95vw)', boxShadow:'0 24px 64px rgba(0,0,0,0.5)', maxHeight:'92vh', overflowY:'auto' }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
                ...
                <button onClick={() => { setSelected(null); setDetail(null); }}
                  style={{ background:'var(--surf2)', border:'1px solid rgba(79,111,250,0.22)', borderRadius:8, width:32, height:32, cursor:'pointer', color:'var(--dim)', fontSize:15, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
              </div>
                  ...
                  {d.signals.length > 0 && (
                    <>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:14 }}>
                        {d.signals.map((s, i) => (
                          <div key={i} style={{ fontSize:12, padding:'7px 12px', background:'var(--surf2)', borderRadius:8, border:'1px solid rgba(79,111,250,0.22)',
