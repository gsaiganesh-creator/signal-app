...
const INP: React.CSSProperties = {
  ...
  border:'1px solid rgba(255,255,255,0.08)', color:'var(--txt)', fontSize:13,
...
function NewStrategyModal({ token, userId, onDone, onClose }: { token: string; userId: string; onDone(): void; onClose(): void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.6)' }}>
      <div style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:28, width:'min(420px,90vw)' }}>
        ...
        <div style={{ display:'flex', gap:10, marginTop:4 }}>
          <button onClick={onClose} style={{ flex:1, height:40, borderRadius:9, background:'transparent', border:'1px solid rgba(255,255,255,0.08)', color:'var(--dim)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>Cancel</button>
...
function NewTradeModal({ strategy, token, userId, onDone, onClose }: { strategy: Strategy; token: string; userId: string; onDone(): void; onClose(): void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.6)' }}>
      <div style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:28, width:'min(400px,90vw)' }}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={fetchPrice} disabled={fetching} style={{ height:40, padding:'0 14px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', color:'var(--dim)', fontSize:12, cursor:'pointer', fontFamily:'inherit', flexShrink:0 }}>
              {fetching ? '…' : 'Fetch'}
            </button>
          </div>
        </div>
...
function CloseTradeModal({ trade, token, onDone, onClose }: { trade: Trade; token: string; onDone(): void; onClose(): void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.6)' }}>
      <div style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:28, width:'min(360px,90vw)' }}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={fetchCurrent} disabled={fetching} style={{ height:40, padding:'0 14px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', color:'var(--dim)', fontSize:12, cursor:'pointer', fontFamily:'inherit', flexShrink:0 }}>
              {fetching ? '…' : 'Live'}
            </button>
          </div>
        </div>
        ...
        {err && <div style={{ fontSize:12, color:'var(--red)', marginBottom:12 }}>{err}</div>}
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onClose} style={{ flex:1, height:40, borderRadius:9, background:'transparent', border:'1px solid rgba(255,255,255,0.08)', color:'var(--dim)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>Cancel</button>
  ...
  if (!loading && strategies.length === 0) {
    return (
      <div style={{ position:'relative' }}>
        ...
        <div style={{ textAlign:'center', padding:'64px 24px', background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px dashed var(--bdr)', borderRadius:16 }}>
    ...
    <div style={{ position:'relative' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button onClick={() => setShowNew(true)} style={{ height:36, padding:'0 14px', borderRadius:9, background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', color:'var(--txt)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>+ Strategy</button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:24, flexWrap:'wrap', overflowX:'auto', paddingBottom:4 }}>
        ...
        <button onClick={() => setShowNew(true)} style={{ padding:'10px 18px', borderRadius:12, background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px dashed var(--bdr)', cursor:'pointer', fontFamily:'inherit', flexShrink:0 }}>
      ...
      {loading ? (
        ...
        <div className="paper-main-grid">
          ...
          <div>
            ...
            <div className="paper-stats-grid">
              {[
                ...
                <div key={c.label} style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:13, padding:16 }}>
            ...
            {/* Equity curve */}
            <div style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:18, marginBottom:16 }}>
              ...
              {[
                ...
                <div key={p.label} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 0', borderBottom:'1px solid var(--bdr)' }}>
                  ...
                  <input type="number" value={p.val} onChange={e => p.set(+e.target.value)} min={p.min} max={p.max} step={p.step}
                    style={{ height:34, padding:'0 12px', borderRadius:8, background:'var(--surf2)', border:'1px solid rgba(255,255,255,0.08)', color:'var(--txt)', fontSize:13, fontWeight:700, fontFamily:'inherit', outline:'none', width:90, textAlign:'right' }}/>
                </div>
              ))}
          ...
          <div>
            ...
            {/* Strategy summary */}
            <div style={{ background:'linear-gradient(145deg,rgba(13,25,42,0.96),rgba(6,11,24,0.92))', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:18, marginBottom:14 }}>
