    ...
    <>
      ...
      <div className="signals-filters" style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <div style={{ position:'relative', flex:'1 1 220px', maxWidth:340 }}>
          ...
          <input
            ...
            style={{ width:'100%', height:38, paddingLeft:36, paddingRight:12, borderRadius:10, border:'1px solid var(--card-bdr)', background:'var(--card-bg)', color:'var(--txt)', fontSize:13, fontFamily:'inherit', boxSizing:'border-box' }}
          />
          {showDropdown && searchResults.length > 0 && (
            <div style={{ position:'absolute', top:42, left:0, right:0, background:'var(--card-bg)', border:'1px solid var(--card-bdr)', borderRadius:10, zIndex:200, boxShadow:'0 8px 32px rgba(0,0,0,0.3)', overflow:'hidden' }}>
        ...
        <button onClick={load} style={{ height:38, padding:'0 16px', borderRadius:10, border:'1px solid var(--card-bdr)', background:'var(--card-bg)', color:'var(--dim)', fontSize:13, cursor:'pointer', fontFamily:'inherit', marginLeft:'auto' }}>
          🔄 Refresh
        </button>
      </div>
      ...
      {mlLoading && (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {[1,2,3,4].map(i => (
            <div key={i} style={{ height:90, borderRadius:14, background:'var(--card-bg)', border:'1px solid var(--card-bdr)', animation:'pulse 1.5s infinite', opacity:0.7 }}/>
          ))}
        </div>
      )}
                ...
                <div>
                  ...
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    {[
                      ...
                      <span key={t} style={{ fontSize:10, padding:'2px 7px', borderRadius:5, background:'var(--surf2)', color:'var(--dim)', border:'1px solid var(--card-bdr)' }}>{t}</span>
                    ))}
                  </div>
                </div>
