...
interface AdminData { summary: Summary; users: UserRow[]; generated_at: string }
        ...
        {s && (
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12, marginBottom:24 }}>
              {[
                { label:'Total Users',       val: s.total_users,       sub: `${s.confirmed} confirmed` },
                { label:'Active (7 days)',   val: s.active_7d,         sub: `of ${s.total_users} total` },
                { label:'India Invested',    val: fmt(s.total_india_invested), sub: `${s.total_holdings} holdings` },
                { label:'US Invested',       val: `$${(s.total_us_invested).toLocaleString('en-US',{maximumFractionDigits:0})}`, sub: `${s.total_portfolios} portfolios` },
                { label:'RSU / ESPP Grants', val: s.total_rsu_grants,  sub: 'across all users' },
              ].map(k => (
                <div key={k.label} style={card}>
                  ...
                  <div style={{ fontSize:26, fontWeight:900, letterSpacing:-0.5 }}>{k.val}</div>
